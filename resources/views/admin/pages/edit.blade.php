...
@section('content')
    <div class="bg-white rounded-xl border border-gray-200 p-6 max-w-3xl">
        <form method="POST" action="{{ route('admin.pages.update', $page) }}">
            @csrf @method('PUT')
            <div class="space-y-5">
                @include('admin._form_components', ['name' => 'title', 'label' => 'Title', 'required' => true, 'value' => $page->title, 'placeholder' => 'Page title'])
                @include('admin._form_components', ['name' => 'content', 'label' => 'Content', 'type' => 'summernote', 'value' => $page->content])
                @include('admin._form_components', ['name' => 'published', 'label' => 'Status', 'type' => 'checkbox', 'value' => $page->published, 'placeholder' => 'Published'])
