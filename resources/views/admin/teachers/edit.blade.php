...
@section('title', 'Edit Teacher - Admin')
    ...
    <div class="bg-white rounded-xl border border-gray-200 p-6 max-w-3xl">
        <form method="POST" action="{{ route('admin.teachers.update', $teacher) }}" enctype="multipart/form-data">
            ...
            <div class="space-y-5">
                ...
                @include('admin._form_components', ['name' => 'name', 'label' => 'Name', 'required' => true, 'value' => $teacher->name, 'placeholder' => 'Full name'])
                @include('admin._form_components', ['name' => 'title', 'label' => 'Title', 'value' => $teacher->title, 'placeholder' => 'e.g. Mathematics Teacher'])
                @include('admin._form_components', ['name' => 'credentials', 'label' => 'Credentials', 'value' => $teacher->credentials, 'placeholder' => 'e.g. M.Sc., B.Ed.'])
                @include('admin._form_components', ['name' => 'sort_order', 'label' => 'Sort Order', 'type' => 'number', 'value' => $teacher->sort_order, 'placeholder' => '0'])
                @include('admin._form_components', ['name' => 'image', 'label' => 'Profile Image', 'type' => 'file', 'value' => $teacher->image])
